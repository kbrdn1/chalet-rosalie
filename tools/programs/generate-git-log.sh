...
show_usage() {
  echo "${BLUE}Usage:${NC} $0 <branch-name> [limit] [output-file]"
  echo "${BLUE}Example:${NC} $0 main 10 changelog.txt"
  exit 1
}
...
validate_branch() {
  ...
  if ! git show-ref --verify --quiet refs/heads/$branch; then
    echo "${RED}Error: Branch '$branch' does not exist${NC}"
    exit 1
  fi

  echo "${GREEN}Using branch: ${branch}${NC}" >&2
}
...
set_limit_param() {
  ...
  if [ ! -z "$limit_value" ] && [[ "$limit_value" =~ ^[0-9]+$ ]]; then
    ...
    echo "${GREEN}Limiting to $limit_value commits${NC}" >&2
  else
    echo "${YELLOW}No limit specified, showing all commits${NC}" >&2
  fi
...
if [ ! -z "$3" ]; then
  OUTPUT_FILE="$3"
  echo "${GREEN}Output will be written to: $OUTPUT_FILE${NC}" >&2
fi
...
echo "${BLUE}Generating git log...${NC}" >&2
get_git_log "$BRANCH" "$LIMIT" > "$OUTPUT_FILE"

if [ $? -eq 0 ]; then
  echo "${GREEN}Git log successfully generated in $OUTPUT_FILE${NC}" >&2
else
  echo "${RED}Failed to generate git log${NC}" >&2
  exit 1
fi
